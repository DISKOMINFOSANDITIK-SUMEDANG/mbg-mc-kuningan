...
interface SchoolReportsTabProps {
  schoolId: string;
}
...
export default function SchoolReportsTab({ schoolId }: SchoolReportsTabProps) {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch(`/api/schools/${schoolId}/reports?date=${selectedDate}`);
        ...
        // Fetch location name for the report if it has coordinates
        if (data.reports && data.reports.length > 0 && data.reports[0].latitude && data.reports[0].longitude) {
        ...
        console.error('Error fetching school reports:', err);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-2xl font-bold text-gray-900">Riwayat Laporan Sekolah</h3>
            </div>
            <p className="text-sm text-gray-600 ml-12">
              Laporan harian yang sudah diupload oleh sekolah ini
            </p>
          </div>
      ...
      {reports.length === 0 ? (
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-dashed border-gray-300 rounded-2xl p-16 text-center">
          ...
          <p className="text-gray-600 max-w-md mx-auto">
            Sekolah ini belum melaporkan pada tanggal {formatDate(selectedDate)}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
            ...
            >
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  <div className="w-14 h-14 bg-gradient-to-br from-green-500 to-green-600 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-green-500/30">
                    <IconPhoto className="h-7 w-7 text-white" />
                  </div>
                  <div className="text-left flex-1">
                    <h4 className="font-bold text-gray-900 text-lg mb-2">
                      Laporan {formatDate(report.report_date)}
                    </h4>
                    <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                      ...
                      <span className="px-3 py-1 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg text-xs font-semibold shadow-sm">
                        Terverifikasi
                      </span>
                    </div>
                  </div>
                ...
                <div className="border-t-2 border-blue-100 bg-gradient-to-br from-gray-50 to-blue-50/30">
                  <div className="p-8 space-y-6">
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <h4 className="text-lg font-bold text-orange-900 mb-2 flex items-center gap-2">
                              <span>Laporan Menu Rapel</span>
                            </h4>
                            <p className="text-orange-800 text-sm leading-relaxed mb-3">
                              Ini adalah laporan menu yang mencakup periode rapel dari <strong>{formatDate(report.rapel_start_date)}</strong> hingga <strong>{formatDate(report.rapel_end_date)}</strong>.
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div className="bg-white rounded-2xl p-6 border-2 border-green-100 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          {report.device_timestamp && (
                            <div>
                              <span className="text-gray-600">Waktu Perangkat:</span>
                              <p className="text-gray-900 font-medium">
                                {formatTime(report.device_timestamp)}
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="bg-white rounded-2xl p-6 border-2 border-purple-100 shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <h5 className="font-bold text-gray-900 text-lg">Lokasi Upload</h5>
                        </div>
                        <div className="space-y-2 text-sm">
                          {report.latitude && report.longitude ? (
                            ...
                            <p className="text-gray-500 text-sm">Data lokasi tidak tersedia</p>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {/* Menu Photo */}
                        <div className="bg-white rounded-2xl border-2 border-blue-100 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 group">
                          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-5 py-3 border-b-2 border-blue-100">
