...
import { IconArrowDown, IconPackage, IconSearch, IconCalendar, IconUser, IconFileText, IconEdit, IconTrash, IconX } from '@tabler/icons-react';
...
export default function StockOutPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load stock movements (filtered for 'out' type)
      ...
      if (movementsResponse.ok) {
        ...
        const outMovements = movementsArray.filter((m: StockMovement) => m.movement_type === 'out');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedProduct = products.find(p => p.id === formData.supplier_product_id);
    if (selectedProduct && parseInt(formData.quantity) > selectedProduct.stock) {
      alert(`Jumlah stok keluar (${formData.quantity}) melebihi stok tersedia (${selectedProduct.stock})`);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        alert(data.error || 'Gagal mencatat stok keluar');
      }
    } catch (error) {
      console.error('Error submitting stock out:', error);
      alert('Gagal mencatat stok keluar');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await loadData();
        alert('Stok keluar berhasil diperbarui');
      } else {
        const data = await response.json();
        alert(data.error || 'Gagal memperbarui stok keluar');
      }
    } catch (error) {
      console.error('Error updating stock out:', error);
      alert('Gagal memperbarui stok keluar');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (movementId: string) => {
    if (!confirm('Apakah Anda yakin ingin menghapus transaksi stok keluar ini? Stok produk akan dikembalikan.')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        await loadData();
        alert('Stok keluar berhasil dihapus dan stok produk telah dikembalikan');
      } else {
        const data = await response.json();
        alert(data.error || 'Gagal menghapus stok keluar');
      }
    } catch (error) {
      console.error('Error deleting stock out:', error);
      alert('Gagal menghapus stok keluar');
    }
  };
    ...
    <CMSLayout>
      <ClientOnly fallback={<PageLoadingState message="Memuat halaman..." />}>
        <div className="space-y-6">
          {/* Header */}
          <div className="bg-gradient-to-r from-red-600 to-pink-600 -mx-6 -mt-6 px-6 py-8 text-white">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-red-100 mt-1">
                  Catat pengurangan stok produk komoditas
                </p>
              </div>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    min="1"
                    max={products.find(p => p.id === formData.supplier_product_id)?.stock || undefined}
                  ...
                  {formData.supplier_product_id && (
                    <p className="text-xs text-gray-500 mt-1">
                      Maksimal: {products.find(p => p.id === formData.supplier_product_id)?.stock || 0} {products.find(p => p.id === formData.supplier_product_id)?.commodities?.unit}
                    </p>
                  )}
                </div>
              ...
              <div className="flex justify-end">
                <button
                  type="submit"
                  disabled={submitting}
                  className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-red-600 to-pink-600 text-white rounded-lg hover:from-red-700 hover:to-pink-700 transition-all duration-200 font-semibold shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {submitting ? (
                    <>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h2 className="text-lg font-bold text-gray-900">Riwayat Stok Keluar</h2>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-gray-600">
                  {searchQuery ? 'Tidak ada data yang sesuai dengan pencarian' : 'Mulai catat stok keluar produk Anda'}
                </p>
              </div>
            ) : (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredMovements.map((movement) => (
                      <tr key={movement.id} className="hover:bg-red-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-bold bg-red-100 text-red-800 border border-red-300">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-600">
                            {movement.previous_stock} → <span className="font-bold text-red-700">{movement.new_stock}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
          ...
          {showEditModal && editingMovement && (
            <div className="fixed inset-0 bg-white/30 backdrop-blur-md flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <div className="sticky top-0 bg-gradient-to-r from-red-600 to-pink-600 text-white px-6 py-4 flex items-center justify-between rounded-t-xl">
                  <h3 className="text-xl font-bold">Edit Stok Keluar</h3>
                  <button
                    onClick={() => {
                      setShowEditModal(false);
                      setEditingMovement(null);
                ...
                <form onSubmit={handleUpdate} className="p-6 space-y-4">
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-red-600 to-pink-600 text-white rounded-lg hover:from-red-700 hover:to-pink-700 transition-all duration-200 font-semibold shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {submitting ? (
                        <>
                          <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
